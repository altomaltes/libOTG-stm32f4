...
dword Manifest_State = Manifest_complete;
/* Data Management variables */
static dword wBlockNum = 0, wlength = 0;
static dword Pointer = APP_DEFAULT_ADD;  /* Base Address to Erase, Program or Read */
static volatile dword  usbd_dfu_AltSet = 0;
...
static void DFU_Req_DNLOAD(void *pdev, USB_SETUP_REQ *req)
{
  ...
  if (req->wLength > 0)
  {
    if ((DeviceState == STATE_dfuIDLE) || (DeviceState == STATE_dfuDNLOAD_IDLE))
    {
      ...
      USBD_CtlPrepareRx ((uint8_t*)MAL_Buffer,
                         wlength);
    }

    else /* Unsupported state */
    { USBD_CtlError (pdev, req);       /* Call the error management function (command will be nacked */
    }
  }
...
/**
  ...
  */
static uint8_t  *USBD_DFU_GetCfgDesc (uint8_t speed, word *length)
{
...
/**
  ...
  * @retval pointer to descriptor buffer
  */
static uint8_t  *USBD_DFU_GetOtherCfgDesc (uint8_t speed, word *length)
{
...
/**
  ...
  */
static uint8_t* USBD_DFU_GetUsrStringDesc (uint8_t speed, uint8_t index , word *length)
{
