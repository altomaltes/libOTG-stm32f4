...
byte g_hub_buf[32];
byte g_hub_intbuf[CONFIG_USBHOST_MAX_EXTHUBS + 1][CONFIG_USB_ALIGN_SIZE];
...
{ int devno;
  ...
  for ( devno = 0
      ; devno < CONFIG_USBHOST_MAX_EXTHUBS
      ; devno++ )
 ...
 * @retval
 */
static int _usbh_hub_get_hub_descriptor(struct usbh_hub *hub, byte *buffer)
 ...
 * @retval
 */
static int _usbh_hub_get_portstatus(struct usbh_hub *hub, byte port, struct hub_port_status *port_status)
{ struct usb_setup_packet *setup;
  int ret;
...
static int _usbh_hub_set_feature(struct usbh_hub *hub, byte port, byte feature)
{ struct usb_setup_packet *setup;
...
static int _usbh_hub_clear_feature(struct usbh_hub *hub, byte port, byte feature)
{ struct usb_setup_packet *setup;
 ...
 * @retval
 */
static int usbh_hub_get_portstatus(struct usbh_hub *hub, byte port, struct hub_port_status *port_status)
{ struct usb_setup_packet roothub_setup;
  ...
  { setup = &roothub_setup;
    ...
    setup->wIndex = port;
    setup->wLength = 4;
    return usbh_roothub_control(&roothub_setup, (byte *)port_status);
  }
  else
  { return _usbh_hub_get_portstatus(hub, port, port_status);
} }

int usbh_hub_set_feature(struct usbh_hub *hub, byte port, byte feature)
{ struct usb_setup_packet roothub_setup;
  struct usb_setup_packet *setup;
...
int usbh_hub_clear_feature(struct usbh_hub *hub, byte port, byte feature)
{ struct usb_setup_packet roothub_setup;
  struct usb_setup_packet *setup;
...
{ if (child->connected)
  { child->connected = false;
    ...
    for (byte i = 0; i < child->config.config_desc.bNumInterfaces; i++)
 ...
 * @retval
 */
static int usbh_hub_connect( struct usbh_hubport * hport
                           , byte intf )
{ struct usb_endpoint_descriptor *ep_desc;
  ...
  ret = _usbh_hub_get_hub_descriptor(hub, (byte *)&hub->hub_desc);
  if (ret < 0)
  { return ret;
  }
 ...
 * @retval
 */
static int usbh_hub_disconnect(struct usbh_hubport *hport, byte intf)
{ struct usbh_hubport *child;
  ...
  { if (hub->intin)
    ...
    for (byte port = 0; port < hub->hub_desc.bNbrPorts; port++)
      ...
      { ret = usbh_hub_set_feature(hub, port + 1, HUB_PORT_FEATURE_RESET);
        ...
        { if (portchange & HUB_PORT_STATUS_C_RESET)
          ...
          { speed = USB_SPEED_HIGH;
            ...
            { extern byte usbh_get_port_speed(struct usbh_hub * hub, const byte port);

              /* USB3.0 speed cannot get from portstatus, checkout port speed instead */
              byte super_speed = usbh_get_port_speed(hub, port + 1);
...
/**
  ...
  * @retval
  */
void usbh_roothub_thread_wakeup(byte port)
