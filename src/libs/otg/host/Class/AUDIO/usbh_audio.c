  ...
  { if (AUDIO_Handle->stream_out[index].valid == 1U)
    { if (ep_size_out < AUDIO_Handle->stream_out[index].EpSize)
      { ep_size_out = AUDIO_Handle->stream_out[index].EpSize;
        ...
        AUDIO_Handle->headphone.Poll = (byte)AUDIO_Handle->stream_out[index].Poll;
    ...
    { if (ep_size_in < AUDIO_Handle->stream_in[index].EpSize)
      { ep_size_in = AUDIO_Handle->stream_in[index].EpSize;
        ...
        AUDIO_Handle->microphone.Poll = (byte)AUDIO_Handle->stream_out[index].Poll;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_ClassRequest()
{ AUDIO_HandleTypeDef *AUDIO_Handle = (AUDIO_HandleTypeDef *) phost->pActiveClass->pData;
  USBH_StatusTypeDef status = USBH_BUSY;
  USBH_StatusTypeDef req_status = USBH_BUSY;


  switch (AUDIO_Handle->req_state)   /* Switch AUDIO REQ state machine */
  { case AUDIO_REQ_INIT:
    ...
    case AUDIO_REQ_SET_DEFAULT_OUT_INTERFACE:
      ...
      { req_status = USBH_SetInterface(phost,
        ...
        { AUDIO_Handle->req_state = AUDIO_REQ_CS_REQUESTS;
          ...
          if (req_status == USBH_NOT_SUPPORTED)
          { USBH_ErrLog("Control error: AUDIO: Device Set interface request failed");
            status = USBH_FAIL;
          }
          else
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_CSRequest( byte feature, byte channel )
{ AUDIO_HandleTypeDef *AUDIO_Handle = (AUDIO_HandleTypeDef *) phost->pActiveClass->pData;
  USBH_StatusTypeDef status = USBH_BUSY;
  USBH_StatusTypeDef req_status = USBH_BUSY;
  word VolumeCtl, ResolutionCtl;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_HandleCSRequest()
{ USBH_StatusTypeDef status = USBH_BUSY;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_FindAudioStreamingIN( )
{ byte interface, alt_settings;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_FindAudioStreamingOUT()
{ byte interface, alt_settings;
  USBH_StatusTypeDef status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_FindHIDControl()
{ byte interface;
  USBH_StatusTypeDef status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_ParseCSDescriptors()
{ USBH_StatusTypeDef status = USBH_OK;
  ...
  { pdesc = USBH_GetNextDesc((byte *)(void *)pdesc, &ptr);

    switch (pdesc->bDescriptorType)
    { case USB_DESC_TYPE_INTERFACE:
        itf_number = *((byte *)(void *)pdesc + 2U);
        alt_setting = *((byte *)(void *)pdesc + 3U);
...
/**
  ...
  */
static uint32_t USBH_AUDIO_FindLinkedUnit(USBH_HandleTypeDef *phost, byte UnitID)
{ byte Index;
  AUDIO_HandleTypeDef *AUDIO_Handle;
 ...
 */
  for ( Index = 0U
      ; Index < AUDIO_Handle->class_desc.MixerUnitNum
      ; Index ++ )
 ...
 */
  for ( Index = 0U
      ; Index < AUDIO_Handle->class_desc.SelectorUnitNum
      ; Index ++ )
  ...
  for ( Index = 0U
      ; Index < AUDIO_Handle->class_desc.OutputTerminalNum
      ; Index ++ )
 ...
 */
  ...
  { value = USBH_AUDIO_FindLinkedUnit(phost, UnitID);
    ...
    Index = (byte)(value & 0xFFU);
    Type = (byte)((value >> 8U) & 0xFFU);
    UnitID = (byte)((value >> 16U) & 0xFFU);

    switch (Type)
    { case UAC_FEATURE_UNIT:    AUDIO_Handle->microphone.asociated_feature = Index; break;
      case UAC_MIXER_UNIT:      AUDIO_Handle->microphone.asociated_mixer   = Index; break;
      case UAC_SELECTOR_UNIT:   AUDIO_Handle->microphone.asociated_selector= Index; break;
      case UAC_OUTPUT_TERMINAL: AUDIO_Handle->microphone.asociated_terminal= Index; break;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_BuildHeadphonePath()
{ byte UnitID = 0U, Type, Index;
  uint32_t value;
  byte terminalIndex;
  AUDIO_HandleTypeDef *AUDIO_Handle;
 ...
 */
  ...
  { value = USBH_AUDIO_FindLinkedUnit(phost, UnitID);
    ...
    switch (Type)
    { case UAC_FEATURE_UNIT:        AUDIO_Handle->headphone.asociated_feature = Index;        break;
      case UAC_MIXER_UNIT:        AUDIO_Handle->headphone.asociated_mixer = Index;        break;
      case UAC_SELECTOR_UNIT:        AUDIO_Handle->headphone.asociated_selector = Index;        break;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AC_SetCur( byte subtype
                                        , byte feature
                                        , byte controlSelector
                                        , byte channel
                                        , word length )
{ word wValue = 0U, wIndex = 0U, wLength = 0U;
  byte UnitID, InterfaceNum;
  AUDIO_HandleTypeDef *AUDIO_Handle;
  AUDIO_Handle = (AUDIO_HandleTypeDef *) phost->pActiveClass->pData;
  USBH_StatusTypeDef ret = USBH_OK;
  ...
  { case UAC_INPUT_TERMINAL:
      ...
      wIndex = (word)((uint32_t)UnitID << 8U) | (word)InterfaceNum;
    ...
    case UAC_FEATURE_UNIT:
      ...
      wValue = (word)((uint32_t)controlSelector << 8U) | (word)channel;
      wLength = length;
      break;
  ...
  return (USBH_CtlReq(phost, (byte *)(void *)(AUDIO_Handle->mem), wLength));
}
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AC_GetCur( byte subtype
                                        , byte feature
                                        , byte controlSelector
                                        , byte channel
                                        ,  word length )
{ word wValue = 0U, wIndex = 0U, wLength = 0U;
  byte UnitID = 0U, InterfaceNum = 0U;
  AUDIO_HandleTypeDef *AUDIO_Handle;
  AUDIO_Handle = (AUDIO_HandleTypeDef *) phost->pActiveClass->pData;
  USBH_StatusTypeDef ret = USBH_OK;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AC_GetMax( byte subtype
                                        , byte feature
                                        , byte controlSelector
                                        , byte channel
                                        , word length )
...
{ word wValue, wIndex, wLength;
  ...
  return (USBH_CtlReq(phost, (byte *)buff, wLength));
  ...
  { case AUDIO_CONTROL_INIT:
      ...
      { AUDIO_Handle->control.timer = phost->Timer;
        (void)USBH_InterruptReceiveData(phost,
                                        (byte *)(void *)(AUDIO_Handle->mem),
                                        (byte)AUDIO_Handle->control.EpSize,
                                        AUDIO_Handle->control.Pipe);
    ...
    case AUDIO_CONTROL_CHANGE:
      ...
      { attribute = LE16(&AUDIO_Handle->mem[0]);
        if (USBH_AUDIO_SetControlAttribute(phost, (byte)attribute) == USBH_BUSY)
        { break;
        }
      }
...
{ USBH_StatusTypeDef status = USBH_BUSY;
  ...
  { case AUDIO_PLAYBACK_INIT:
    ...
    case AUDIO_PLAYBACK_SET_EP_FREQ:
      ...
      buff = (byte *)AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as].FormatTypeDesc->tSamFreq[0];
    ...
    case AUDIO_PLAYBACK_SET_EP:
      buff = (byte *)(void *)&AUDIO_Handle->headphone.frequency;
      status = USBH_AUDIO_SetEndpointControls(phost, AUDIO_Handle->headphone.Ep, buff);
      if (status == USBH_OK)
      { AUDIO_Handle->play_state = AUDIO_PLAYBACK_IDLE;
...
{ USBH_StatusTypeDef Status = USBH_BUSY;
  AUDIO_HandleTypeDef *AUDIO_Handle;
  byte              index;
  byte              change_freq = FALSE;
  uint32_t             freq_min, freq_max;
  byte              num_supported_freq;
...
/**
  ...
  * @retval USBH Status
  */
USBH_StatusTypeDef USBH_AUDIO_Play(USBH_HandleTypeDef *phost, byte *buf, uint32_t length)
{ USBH_StatusTypeDef Status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
...
/**
  ...
  * @retval USBH Status
  */
USBH_StatusTypeDef USBH_AUDIO_ChangeOutBuffer(USBH_HandleTypeDef *phost, byte *buf)
{ USBH_StatusTypeDef Status = USBH_FAIL;
  AUDIO_HandleTypeDef *AUDIO_Handle;
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef USBH_AUDIO_SetControlAttribute(USBH_HandleTypeDef *phost, byte attrib)
{ USBH_StatusTypeDef status = USBH_BUSY;
  AUDIO_HandleTypeDef *AUDIO_Handle;
  ...
  if (AUDIO_SetVolume(phost,
                      AUDIO_Handle->temp_feature,
                      (byte)AUDIO_Handle->temp_channels,
                      (word)AUDIO_Handle->headphone.attribute.volume) != USBH_BUSY)
  {
...
/**
  ...
  * @retval USBH Status
  */
static USBH_StatusTypeDef AUDIO_SetVolume(USBH_HandleTypeDef *phost, byte feature, byte channel, word volume)
{ USBH_StatusTypeDef status = USBH_BUSY;
  AUDIO_HandleTypeDef *AUDIO_Handle;
