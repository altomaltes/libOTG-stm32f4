...
typedef byte( *hostClassLink)(byte);
...
{ ENUM_State          EnumState;    /* Enumeration state Machine */
  ...
  byte ( *handleCtrlPkg )( byte epNum ); /* Package arrive manage */
} USBH_HOST_REC;
...
{// dword       XferCnt[ USB_OTG_MAX_TX_FIFOS ];
  ...
  { short reload;                        /* Timer active     */
    ...
    void ( * action )( signed char channel );  /* JACS, action to do on events */
...
void  USBHxferOutControl( dword channel );
schar USBHxferInControl(  dword channel );
