...
#define __LOBYTE(x)                    ((unsigned char)((x) & 0x00FF))
#define __HIBYTE(x)                    ((unsigned char)(((x) & 0xFF00) >>8))

#define FILE_DATE(y, m, d)             (((((unsigned short)(y) - 1980) & 0x7F) << 9) | \
...
/* FATFS DUMMY BYTE -------------------------------------------------------- */
#define RESERVED_BYTE                  ((unsigned char)0x00)

#define FAT_TABLE_ADDR						     0x8003800                /* 14K */
#define FLASH_PAGE_SIZE						     ((unsigned int)0x00000800)   /* FLASH Page Size */
/* SECTOR SIZE ------------------------------------------------------------- */
#define SECTOR_SIZE							       ((unsigned int)0x200)
...
#define FAT1_TABLE_OFFSET              ((unsigned int)0x200)
#define FAT2_TABLE_SECTOR_IDX          (4)
#define FAT2_TABLE_OFFSET              ((unsigned int)0x800)
static unsigned char FATn_TABLE[FATn_TABLE_SIZE] = {0x00, 0x00, 0x00, 0x03, 0x40, 0x00, 0xFF, 0x0F};
